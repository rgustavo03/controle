...
import deleteFornecedor from "../../../services/fornecedores/deleteFornecedor";


export const DelFornecedor = ({ navigate }) => {


  const { getToken } = useSession();

  const { item, updateListFornecedores, closeDelete } = useContext(FornecedoresContext);


  function deleteItem() {
    deleteFornecedor(getToken(), item.id, updateListFornecedores, navigate); // api
    closeThis();
  }
    ...
    <div id="delete-screen" className="h-min w-min bg-white rounded-lg flex flex-col sm:flex-row p-5">
      ...
      <div id="delete-info" className="min-w-[300px] md:w-[420px] flex flex-col gap-3">
        ...
        <div id="delete-top" className="py-2 px-1 sm:px-3 flex flex-col gap-2">
          <h3 className="text-lg font-bold text-gray-600">Remover Fornecedor</h3>
          <div className="flex flex-col md:flex-row items-start md:items-center gap-2">
            <h6 className="text-md text-gray-700">Deseja remover o fornecedor?</h6>
